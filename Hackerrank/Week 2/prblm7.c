...
// A struct to hold each dynamic array (sequence)
typedef struct {
    int *data;
    int size;
    int capacity;
...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    ...
    // Allocate a temporary array to hold results for type 2 queries
    ...
    for (int i = 0; i < queries_rows; i++) {
        ...
        if (qType == 1) {
            ...
            appendArray(&seqList[idx], y);
        } else if (qType == 2) {
            // Retrieve value at position y % size(seqList[idx])
            int size = seqList[idx].size;
            int val = seqList[idx].data[y % size];
            lastAnswer = val;
            answers[answersCount++] = lastAnswer;
        }
    }
